...
from sqlalchemy import CheckConstraint, Date, DateTime, String, Text, UniqueConstraint, func
...
class MailSettings(Base):
    ...
    __table_args__ = (CheckConstraint("id = 'default'", name="ck_mail_settings_single_row"),)
...
class WorkReport(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("work_style in ('office', 'remote')", name="ck_work_reports_work_style"),
    )
    ...
    start_mail_created_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    end_mail_created_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    note: Mapped[str] = mapped_column(Text, default="", nullable=False)
    work_style: Mapped[str] = mapped_column(String(16), default="office", nullable=False)
